...
function getProjects() {
  const test = document.getElementById("form-input-project");
  ...
  function passData(e) {
    if (e.code === "Enter") {
      ...
      if (e.target.value !== "") {
        ...
        test.setCustomValidity("Please enter a Project name.");
      }
    }
  }
}
...
function getDate(modalContent) {
  ...
  function collectDate(e) {
    ...
    let date_date = checkerDomElement("date-date", e.target.childNodes, "id")
      .value;
    ...
    let taskName = checkerDomElement("task-name", domListTasks, "class")
      .textContent;
...
export { getTask, getProjects, getDate };
